...
export class ConviteRepository implements IConviteRepository {
  async criar(convite: Convite): Promise<Convite> {
    const result = await pool.query(
      `INSERT INTO convites (remetenteId, destinatarioId, status)
       VALUES ($1, $2, $3) RETURNING *`,
      [convite.remetenteId, convite.destinatarioId, convite.status ?? "pendente"]
    );
    const row = result.rows[0];
    return new Convite(row.id, row.remetenteId, row.destinatarioId, row.status);
  }
  ...
  async listarPorJogador(jogadorId: number): Promise<Convite[]> {
    const result = await pool.query(
      "SELECT * FROM convites WHERE destinatarioId = $1",
      [jogadorId]
    );
    return result.rows.map(
    (row: ConviteRow) => new Convite(row.id, row.remetenteId, row.destinatarioId, row.status)
    );
  }
}
